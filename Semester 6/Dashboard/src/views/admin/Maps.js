...
import {productionListWind} from "../../lists/ProductionListWind.js";
...
export default function Maps() {
    ...
    const svgIconsMapping = (code) => {
        let iconElement = '';
        switch (code) {
            case ('1'):
                iconElement = svgIcons.rain;
                console.log('this is the element', svgIcons)
                break;
            case ('2'):
                iconElement = svgIcons.cloudy;
                break;
            case ('3'):
                iconElement = svgIcons.cloudy;
                break;
    ...
    const [region, setRegion] = useState('Groningen');
    const [type, setType] = useState(productionListSun);
    ...
    const regionOptions = [
        ...
        {value: 'Hoogezand', label: 'Hoogezand'},
        {value: 'Veendam', label: 'Veendam'},
        {value: 'Stadskanaal', label: 'Stadskanaal'},
        {value: 'Winschoten', label: 'Winschoten'},
        {value: 'Haren', label: 'Haren'},
        {value: 'Delfzijl', label: 'Delfzijl'},
        {value: 'Hoogkerk', label: 'Hoogkerk'},
        {value: 'Appingedam', label: 'Appingedam'},
        {value: 'Leek', label: 'Leek'},
        {value: 'Bedum', label: 'Bedum'},
        {value: 'Ter Apel', label: 'Ter Apel'},
        {value: 'Sappemeer', label: 'Sappemeer'},
        {value: 'Zuidhorn', label: 'Zuidhorn'},
        {value: 'Oude Pekela', label: 'Oude Pekela'},
        {value: 'Winsum', label: 'Winsum'},
        {value: 'Musselkanaal', label: 'Musselkanaal'},
        {value: 'Marum', label: 'Marum'},
        {value: 'Wildervank', label: 'Wildervank'},
        {value: 'Uithuizen', label: 'Uithuizen'},
    ];

    const RegionDropdown = () => (
        <Select className="mt-4" options={regionOptions}
    ...
    const TypeDropdown = () => (
        <Select className="mb-4 "
                styles={SelectStyles()}
                onChange={(e) => typeSelect(e)}
            options={[ {value: productionListSun, label: 'sun'},
                {value: 'oil', label: 'oil'},
                {value: 'gas', label: 'gas'},
                {value: productionListWind, label: 'wind'},
    ...
    const regionSelect = (event) => {
        async function fetchData() {
            const promise = await Weather.regionWeather(event.value);
            ...
            await myPromise
                .then(response => {
                    ...
                    const day24 = response.data.data[24];
                    const day48 = response.data.data[48];
                    const day72 = response.data.data[72];
                    const day96 = response.data.data[96];

                    const data = {
                        current: {
                            date: current.tijd_nl,
                            description: current.samenv,
                            icon: svgIconsMapping(current.ico),
                            temperature: {current: current.temp, min: current.temp, max: current.temp},
                            wind: current.winds,
                            humidity: current.rv,
                        },
                        forecast: [
                            ...
                            {
                                date: day24.tijd_nl,
                                description: day24.samenv,
                                icon: svgIconsMapping(day24.ico),
                                temperature: {day24: day24.temp, min: day24.temp, max: day24.temp},
                                wind: day24.winds,
                                humidity: day24.rv,
                            },
                            {
                                date: day48.tijd_nl,
                                description: day48.samenv,
                                icon: svgIconsMapping(day48.ico),
                                temperature: {day48: day48.temp, min: day48.temp, max: day48.temp},
                                wind: day48.winds,
                                humidity: day48.rv,
                            },
                            {
                                date: day72.tijd_nl,
                                description: day72.samenv,
                                icon: svgIconsMapping(day72.ico),
                                temperature: {day72: day72.temp, min: day72.temp, max: day72.temp},
                                wind: day72.winds,
                                humidity: day72.rv,
                            },
                            {
                                date: day96.tijd_nl,
                                description: day96.samenv,
                                icon: svgIconsMapping(day96.ico),
                                temperature: {day96: day96.temp, min: day96.temp, max: day96.temp},
                                wind: day96.winds,
                                humidity: day96.rv,
                            },
    ...
    return (
        <>
            <div className="flex flex-wrap pt-32 px-12">
